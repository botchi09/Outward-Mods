...
namespace CombatHUD_2
{
    public class DamageLabels : MonoBehaviour
    {
        ...
        internal void Update()
        {
            ...
            {
                if (Time.time - ActiveLabels[z].CreationTime > 2.5f)
                {
            ...
            for (int i = 0; i < SplitScreenManager.Instance.LocalPlayerCount; i++)
            {
                ...
                for (int j = 0; j < LabelHolders.Count; j++)
                {
                    ...
                    else
                    {
                        ...
                        float damageStrength = (float)((decimal)labelInfo.Damage / 50); // set damage "strength"
                        float time = Time.time - labelInfo.CreationTime;
                        var timeOffset = Mathf.Lerp(0.17f, 0.03f, damageStrength) * time;

                        // get world-to-viewport point using game's own visibilty checker
                        var screenPos = camera.WorldToViewportPoint(labelInfo.Target.CenterPosition + new Vector3(0, timeOffset));
                        if (charUI.ViewportPosToSplitScreenPos(ref screenPos))
                        {
                            screenPos += new Vector3
                            (
                                HUDManager.RelativeOffset(45f + labelInfo.ranXpos),
                                HUDManager.RelativeOffset(labelInfo.ranYpos + timeOffset, true)
                            );
        ...
        public void AddDamageLabel(DamageList damageList, Vector3 hitPosition, Character target)
        {
            ...
            DamageLabel label = new DamageLabel
            {
                ...
                Target = target,
                ranXpos = UnityEngine.Random.Range(-25, 25),
                ranYpos = UnityEngine.Random.Range(-10, 10)
            };
